...
type Row = { name: string; result?: VerificationResult; error?: string };
...
function App() {
  ...
  const verifyBatch = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      const nextRows = data.results.map((item: { filename: string; result?: VerificationResult; error?: string }) => ({ name: item.filename, result: item.result, error: item.error }));
